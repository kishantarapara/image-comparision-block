...
const BlockEdit = (props) => {
  ...
  const { beforeImageUrl, afterImageUrl, beforeImageId, afterImageId } = attributes;
  ...
  const handleSliderChange = (event) => {
	...
	return (
		<Fragment>
			<InspectorControls>
				<PanelBody
					title={__('Select Before Image', 'awp')}
					initialOpen={ true }
				>
					<div className="editor-post-featured-image">
						<MediaUploadCheck>
							<MediaUpload
								...
								render={({open}) => (
									<Button 
										className={beforeImageId == 0 ? 'editor-post-featured-image__toggle' : 'editor-post-featured-image__preview'}
										onClick={open}
									>
                    ...
                    {beforeImageId !== 0 && 
						...
						{beforeImageId != 0 && 
							<MediaUploadCheck>
								<Button onClick={removeBefore} isLink isDestructive>{__('Remove image', 'awp')}</Button>
							</MediaUploadCheck>
						}
					</div>
				</PanelBody>
				<PanelBody
					title={__('Select After Image', 'awp')}
					initialOpen={ true }
				>
					<div className="editor-post-featured-image">
						<MediaUploadCheck>
							<MediaUpload
								...
								render={({open}) => (
									<Button 
										className={afterImageId == 0 ? 'editor-post-featured-image__toggle' : 'editor-post-featured-image__preview'}
										onClick={open}
									>
                    ...
                    {afterImageId !== 0 && 
						...
						{afterImageId != 0 && 
							<MediaUploadCheck>
								<Button onClick={removeAfter} isLink isDestructive>{__('Remove image', 'awp')}</Button>
							</MediaUploadCheck>
						}
					</div>
				</PanelBody>
        ...
        <div class="c-compare" style={{"--value": `${sliderPosition}%`}}>
          ...
          <input type="range" class="c-rng c-compare__range" min="0" max="100" value={sliderPosition} onChange={handleSliderChange} />
        </div>
			</div>
		</Fragment>
	);
...
registerBlockType('image-comparison-block/interactive-image-comparison-block', {
  ...
  description: 'This block enables users to compare two images interactively using a slider.',
  ...
  save: (props) => {
    ...
    <div>
        <div class="c-compare" style={{"--value": `${50}%`}}>
          <img class="c-compare__left" src={beforeImageUrl} alt="Before" />
          <img class="c-compare__right" src={afterImageUrl} alt="After" />
          <input type="range" class="c-rng c-compare__range" min="0" max="100" value={50} />
        </div>
		</div>
  )},
});
